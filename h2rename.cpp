	...
	: QWidget(parent, flags)
{
    ...
    connect(insertRuleAction, &QAction::triggered, this, &H2rename::on_pushButton_insertRow_clicked);
    ...
    connect(removeRulesAction, &QAction::triggered, this, &H2rename::on_pushButton_removeRow_clicked);
    ...
    connect(createRuleAction, &QAction::triggered, this, &H2rename::createRule);
	ui.tableView_names->addAction(createRuleAction);

    connect(&RenameRulesModel::globalInstance(), &RenameRulesModel::rulesChanged, &NamesModel::globalInstance(), &NamesModel::computeNewNames);
    connect(&RenameRulesModel::globalInstance(), &RenameRulesModel::rowsRemoved, &NamesModel::globalInstance(), &NamesModel::computeNewNames);

    connect(&NamesModel::globalInstance(), &NamesModel::dataChanged, this, &H2rename::updateNumbers);

    connect(ui.tableView_names->selectionModel(), &QItemSelectionModel::selectionChanged, this, &H2rename::namesSelectionChanged);

    connect(ui.tableView_replacements->selectionModel(), &QItemSelectionModel::selectionChanged, this, &H2rename::replacementsSelectionChanged);

    connect(&NamesModel::globalInstance(), &NamesModel::collisionDetected, this, &H2rename::collisionDetected);

    connect(&NamesModel::globalInstance(), &NamesModel::highlightsChanged,
            &NamesFilterProxyModel::globalInstance(), &NamesFilterProxyModel::highlightsChanged);

    QAction *actionAboutQt = new QAction(tr("Über Qt"),this);
    actionAboutQt->setMenuRole(QAction::AboutQtRole);
    ui.toolButton_About->addAction(actionAboutQt);
    connect(actionAboutQt, &QAction::triggered, this, &QApplication::aboutQt);

    QAction *actionAboutH2rename = new QAction(tr("Über H2rename"),this);
    ui.toolButton_About->addAction(actionAboutH2rename);
    connect(actionAboutH2rename, &QAction::triggered, [this](){
...
void H2rename::readDirs()
{
	if (!readDirThread.isRunning())
	{
        ...
        connect(&readDirThread, &ReadDirThread::finished, &dialog, &QDialog::accept);
        connect(&readDirThread, &ReadDirThread::processing, dialog.ui.label_currentDir, &QLabel::setText);
...
{
    RenameRulesModel::globalInstance().insertRow(ui.tableView_replacements->currentIndex().row()+1);
}
